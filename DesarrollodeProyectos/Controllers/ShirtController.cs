...
public async Task<IActionResult> ShirtAdd(ShirtModel shirtModel)
{
    ...
    return RedirectToAction("ShirtList"); 
}
    ...
    public async Task<IActionResult> ShirtList()
{
    var shirts = await _context.Shirts
        .Where(s => s.IsActive) // Solo mostrar camisas activas
        .Include(s => s.Size)
...
{
    var shirt = await this._context.Shirts
        .Where(s => s.Id == id)
        .FirstOrDefaultAsync();
    ...
    // Convertir la camisa a ShirtModel para mostrar en la vista
    ShirtModel model = new ShirtModel
    {
        Id = shirt.Id,
        Name = shirt.Name,
        ...
        Size = shirt.Size,
        Price = shirt.Price,
        IsActive = shirt.IsActive // Mostrar el estado actual de la camisa
    };
...
public async Task<IActionResult> ShirtDeleted(ShirtModel shirt)
{
    ...
    // Obtener la camisa de la base de datos
    Shirt shirtEntity = await this._context.Shirts
        .Where(s => s.Id == shirt.Id)
        .FirstAsync();

    shirtEntity.IsActive = false;

    // Guardar los cambios en la base de datos
    this._context.Update(shirtEntity);
    await this._context.SaveChangesAsync();

    // Redirigir a la lista de camisas
    return RedirectToAction("ShirtList", "Shirt");
}
